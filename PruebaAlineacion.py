...
# Extraemos la info de la im2
keypoint2, descriptor2 = orb.detectAndCompute(gray_im2, None)

# Dibujamos
im1_display = cv2.drawKeypoints(im1, keypoint1, outImage = np.array([]), color =(255,0,0),
                                flags= cv2.DRAW_MATCHES_FLAGS_DEFAULT)
im2_display = cv2.drawKeypoints(im2, keypoint2, outImage=np.array([]), color=(255, 0, 0),
                                flags=cv2.DRAW_MATCHES_FLAGS_DEFAULT)
...
img_matches = cv2.drawMatches(im1, keypoint1, im2, keypoint2, matches, None)
...
img_perspec = cv2.warpPerspective(im2, h, (ancho, alto))

while True:
    # Mostramos caracteristicas
    cv2.imshow("VIDEO CAPTURA", im2_display)
    cv2.imshow("IMAGEN", im1_display)
    cv2.imshow("COINCIDENCIAS", img_matches)
    cv2.imshow("PERSPECTIVA", img_perspec)
